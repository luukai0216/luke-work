(()=>{
  ...
  const page=/Glyph-Features\.html$/i.test(location.pathname)?glyph:type;
  const translations={...common,...page};
  if(language==='zh-CN'){
    const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);
    const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
    nodes.forEach(node=>{if(node.parentElement?.closest('.weight-control'))return;const key=node.nodeValue.trim();if(translations[key])node.nodeValue=node.nodeValue.replace(key,translations[key])});
    document.title=/Glyph-Features\.html$/i.test(location.pathname)?'字形特征 — TRON 设计语言':'字体系统 — TRON 设计语言';
