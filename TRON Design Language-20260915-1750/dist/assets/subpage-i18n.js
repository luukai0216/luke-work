// 页面判定：原本用 /Glyph-Features\.html$/i 匹配 location.pathname。
// 迁移到干净 URL（/design/glyph-features/）后该正则不再命中，会把 glyph 页
// 误判成 typography —— 切中文时标题会串成「字体系统」。
// 改成不带扩展名、不锚定结尾的匹配，新旧两种 URL 形态都命中。
(()=>{
  const scrollKey=`tron-design-language-scroll:${location.pathname}`;
  const savedScroll=sessionStorage.getItem(scrollKey);
  const restoreY=savedScroll===null?null:Math.max(0,Number(savedScroll)||0);
  window.__restoringLanguageScroll=restoreY!==null;
  if(savedScroll!==null){
    sessionStorage.removeItem(scrollKey);
    const restoreScroll=()=>scrollTo(0,restoreY);
    requestAnimationFrame(()=>requestAnimationFrame(restoreScroll));
    addEventListener('load',()=>{restoreScroll();requestAnimationFrame(()=>requestAnimationFrame(()=>{window.__restoringLanguageScroll=false;document.querySelector('.nav')?.classList.remove('is-hidden')}))},{once:true});
  }
  ...
  const page=/glyph-features/i.test(location.pathname)?glyph:type;
  const translations={...common,...page};
  if(language==='zh-CN'){
    const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);
    const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);
    nodes.forEach(node=>{if(node.parentElement?.closest('.weight-control,.ts-scale-row[data-scale^="body-"]'))return;const key=node.nodeValue.trim();if(translations[key])node.nodeValue=node.nodeValue.replace(key,translations[key])});
    const usageRoleTranslations={Standard:'标准',Emphasis:'强调',Primary:'主要',Hero:'主视觉',Body:'正文',Actions:'操作','Use sparingly':'谨慎使用',Default:'默认',Important:'重要','Key value':'关键数值','Display value':'展示数值'};
    document.querySelectorAll('.ts-usage-role').forEach(role=>{const key=role.textContent.trim();if(usageRoleTranslations[key])role.textContent=usageRoleTranslations[key]});
    const pageTitle=document.querySelector(/glyph-features/i.test(location.pathname)?'.hero-title':'.ts-title');
    if(pageTitle)pageTitle.innerHTML=/glyph-features/i.test(location.pathname)?'Glyph<br>Features':'Typography<br>System';
    document.title=/glyph-features/i.test(location.pathname)?'字形特征 — TRON 设计语言':'字体系统 — TRON 设计语言';
  ...
  window.setSiteLanguage=next=>{sessionStorage.setItem(scrollKey,String(scrollY));localStorage.setItem('tron-design-language',next);location.reload()};
  ...
  document.querySelectorAll('.site-footer-language button').forEach(button=>{const active=button.dataset.language===language;button.classList.toggle('is-active',active);button.setAttribute('aria-pressed',String(active));button.addEventListener('click',()=>window.setSiteLanguage(button.dataset.language))});
})();
