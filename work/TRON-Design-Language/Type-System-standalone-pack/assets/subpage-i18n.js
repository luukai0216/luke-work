(()=>{
  ...
  if(language==='zh-CN'){
    ...
    nodes.forEach(node=>{if(node.parentElement?.closest('.weight-control,.numeral-title,.glyph,.preview-char,.tabular-row,.proportional-num,.align-row,.ot-card .sample,.ot-card .tag'))return;const key=node.nodeValue.trim();if(translations[key])node.nodeValue=node.nodeValue.replace(key,translations[key])});
